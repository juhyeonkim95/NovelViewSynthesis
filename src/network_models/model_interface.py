...
class ModelInterface:
    def __init__(self, name, image_size, load_file_name=None):
        self.name = name
        self.model = Model()
    ...
    def save_model(self, data_name, file_name):
        ...
        # with open("model_structure_%s.json" % file_name, "w") as json_file:
        #     json_file.write(model_json)
        self.model.save_weights("%s_%s_%s.h5" % (data_name, self.name, file_name))
        print("Saved model to disk")

    def load_model(self, file_name):
        # json_file = open("model_structure_%s.json" % file_name, "r")
